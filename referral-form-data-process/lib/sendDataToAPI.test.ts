...
describe("#sendDataToAPI", () => {
    const testReferrerQuestion = "TEMP: Question for getting the referrer of the form";
    const testSupportQuestion = "TEMP: Question for getting the requested support";
    const testClientsQuestion = "TEMP: Question for getting the people being referred";
    const testReferrerAnswer = "test name";
    const testSupportAnswer = "test support";
    const testClientsAnswer = "test client";
    const testFormData = { [testReferrerQuestion]: [testReferrerAnswer], [testSupportQuestion]: [testSupportAnswer], [testClientsQuestion]: [testClientsAnswer] };
    ...
    it("should call the service API with the correct parameters", async () => {
        ...
        expect(mockedAxios.post.mock.calls[0][2]?.headers).toEqual({ 'x-api-key': testAPIKey })
    })
})