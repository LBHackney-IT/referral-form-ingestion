...
describe("#createDocumentFromTemplate", () => {
  ...
  it("should call google drive", async () => {
    ...
    expect(google.drive).toBeCalledWith({
      version: googleDriveVersion,
      auth: mockAuth,
    });
  });
  ...
  it("should call google docs", async () => {
    ...
    expect(google.docs).toBeCalledWith({
      version: googleDocsVersion,
      auth: mockAuth,
    });
  });
  ...
  it("should call google document's batch update with the correct parameters", async () => {
    ...
    expect(mockBatchUpdate).toBeCalledWith({
      documentId: copyOfOriginalTemplateId,
      requestBody: {
        requests: [
          {
            replaceAllText: {
              containsText: { matchCase: true, text: "{{testKey}}" },
              replaceText: "testValue",
            },
          },
        ],
      },
